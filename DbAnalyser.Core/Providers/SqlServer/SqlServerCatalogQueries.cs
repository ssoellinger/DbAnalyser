...
public class SqlServerCatalogQueries : ICatalogQueries
{
    ...
    public async Task<List<JobRow>> GetJobsAsync(IDbProvider provider, string databaseName, CancellationToken ct)
    {
        try
        {
            var data = await provider.ExecuteQueryAsync($"""
